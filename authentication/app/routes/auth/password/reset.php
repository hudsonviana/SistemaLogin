...
$app->get('/password-reset', $guest(), function() use ($app) {
    ...
    if (!$user->recuperar_hash) {
        $app->response->redirect($app->urlFor('home'));
    }

    if (!$app->hash->hashCheck($user->recuperar_hash, $hashedIdentifier)) {
        $app->response->redirect($app->urlFor('home'));
    }
...
$app->post('/password-reset', $guest(), function() use ($app) {
    ...
    if ($v->passes()) {
        ...
        $app->flash('global', 'Sua senha foi redefinida e agora você pode fazer o login.');
        $app->response->redirect($app->urlFor('home'));
    }
